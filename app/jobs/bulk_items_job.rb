class BulkItemsJob < ApplicationJob
  queue_as :default

  def perform(operation, items_attributes, callback_url = nil)
    case operation.to_sym
    when :create
      result = BulkOperationsService.bulk_create_items(items_attributes)
    when :update
      result = BulkOperationsService.bulk_update_items(items_attributes)
    else
    ...
    Rails.logger.info("BulkItemsJob #{operation} completed: #{result.to_json}")
